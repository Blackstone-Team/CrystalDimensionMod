...
public class CrystalFieldsBiome extends CrystalDimensionModElements.ModElement {
	public static Biome biome;
	public CrystalFieldsBiome(CrystalDimensionModElements instance) {
		super(instance, 36);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.PLAINS).depth(0.1f).scale(0.3f)
						.temperature(0.5f).downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("crystal_dimension:crystal_fields"));
			}
		}
	}
	@Override
